...
@Component({
  selector: 'app-cele3',
  templateUrl: './cele3.component.html',
  styleUrls: ['./cele3.component.css',
...
export class Cele3Component {
  ...
  defaultyear()
  {
    ...
    if (this.currentYear == 2022) {
      this.googleDriveService.setFolderId("1ZDef0GB5nHXMg7jcRmm_3X7hTApEDJp3");
      this.yearselcted = true;
      this.validyear=true

    }
    if (this.currentYear == 2023) {
      this.googleDriveService.setFolderId("1wvJw8iFvPfkahoXqIQ1KbC8yM3lEqmuf");
      this.yearselcted = true;
      this.validyear=true
    ...
    if (this.currentYear == 2024) {
      this.googleDriveService.setFolderId("1olP5L-DYdy6BVeLeAHx3GHSn65G4fYwY");
      this.validyear = true

    }
    if (this.currentYear == 2025) {
      this.googleDriveService.setFolderId("1t1jsALZM_uaN8wb5RfKApKIolnflM-gI");
      this.validyear = true

    }
    if (this.currentYear == 2026) {
      this.googleDriveService.setFolderId("1oRPG-dinY4BdJk22aQ3VrbDQ7o6uzIJh");
      this.validyear = true
  ...
  fetchBanner(): void {
    ...
    this.googleDriveService.setFolderId('1CAnsUkrpiyA7-nnQE4CXWpY8WAfPNmxa')
    this.portrait().subscribe((response: any) => {
      this.Banner = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1LW60aFZt_w7LqVqU41-YhhFf9tFeu2D_')
  ...
  portrait(): Observable<any> {
    const folderid='1CAnsUkrpiyA7-nnQE4CXWpY8WAfPNmxa'
  ...
  landscape(): Observable<any> {
    const folderid='1LW60aFZt_w7LqVqU41-YhhFf9tFeu2D_'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here
  ...
  banner(): Observable<any> {
    const folderid='1wvJw8iFvPfkahoXqIQ1KbC8yM3lEqmuf'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here

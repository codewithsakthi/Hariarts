...
@Component({
  selector: 'app-cele2',
  templateUrl: './cele2.component.html',
  styleUrls: ['./cele2.component.css',
...
export class Cele2Component {
  ...
  defaultyear()
  {
    ...
    if (this.currentYear == 2022) {
      this.googleDriveService.setFolderId("1KFgS3ZN0pjRFDjWx9ZmAHlj0u_j4kjQC");
      this.yearselcted = true;
      this.validyear=true

    }
    if (this.currentYear == 2023) {
      this.googleDriveService.setFolderId("1mVe2CA4_nT9JBODVvSqjzWzHGtXe-L7e");
      this.yearselcted = true;
      this.validyear=true
    ...
    if (this.currentYear == 2024) {
      this.googleDriveService.setFolderId("1IUuEuFivNzo5M5QtbwAD3NEAE-iJQAaV");
      this.validyear = true

    }
    if (this.currentYear == 2025) {
      this.googleDriveService.setFolderId("1ECfsaaofdjkqYPAAeIjh72uS65vajxWi");
      this.validyear = true

    }
    if (this.currentYear == 2026) {
      this.googleDriveService.setFolderId("1cPlGwNdyiMy7mltROHM5iWvL7fwFNQjF");
      this.validyear = true

    }
  }
  fetchBanner(): void {
    this.googleDriveService.setFolderId('1bL-ybZ43Z0HUFiIpu7CY03fw_zo3wCJq')
    this.banner().subscribe((response: any) => {
      this.data = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1smF0ZftwTdP-nSChtjQ_642C-VO2-WAe')
    this.portrait().subscribe((response: any) => {
      this.Banner = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1I17z81al23pSIgFfZ_oyu98SdSIBtPOp')
  ...
  portrait(): Observable<any> {
    const folderid='1smF0ZftwTdP-nSChtjQ_642C-VO2-WAe'
  ...
  landscape(): Observable<any> {
    const folderid='1I17z81al23pSIgFfZ_oyu98SdSIBtPOp'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here
  ...
  banner(): Observable<any> {
    const folderid='1bL-ybZ43Z0HUFiIpu7CY03fw_zo3wCJq'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here

...
@Component({
  selector: 'app-cele1',
  templateUrl: './cele1.component.html',
  styleUrls: ['./cele1.component.css',
...
export class Cele1Component {
  ...
  defaultyear()
  {
    ...
    if (this.currentYear == 2022) {
      this.googleDriveService.setFolderId("1xFqmOe9doADX_GmmoRHC1dfS4LA06u2U");
      this.yearselcted = true;
      this.validyear=true

    }
    if (this.currentYear == 2023) {
      this.googleDriveService.setFolderId("1rabEW38f5l6JvVC14Ts8_oKaKJb7jg-c");
      this.yearselcted = true;
      this.validyear=true
    ...
    if (this.currentYear == 2024) {
      this.googleDriveService.setFolderId("1zonVTL-iPJ-mShNXdecVpeh1uBnc-fHW");
      this.validyear = true

    }
    if (this.currentYear == 2025) {
      this.googleDriveService.setFolderId("1DPPllnozSosxnw2c9kJyvgKTMl7thqwC");
      this.validyear = true

    }
    if (this.currentYear == 2026) {
      this.googleDriveService.setFolderId("1r1VV9LgaWs3GYc6RWDleGlnPtx1m7eMC");
      this.validyear = true

    }
  }
  fetchBanner(): void {
    this.googleDriveService.setFolderId('1v7FUHc9sFSQPMcp9SynXM_qgdPO18SSG')
    this.banner().subscribe((response: any) => {
      this.data = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1BjK5mX7oEthbqq5EluwhbwP0ERXhZBPN')
    this.portrait().subscribe((response: any) => {
      this.Banner = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1GQoLl1UUXZOPKT8L_ykWf05nSlInyzoB')
  ...
  portrait(): Observable<any> {
    const folderid='1BjK5mX7oEthbqq5EluwhbwP0ERXhZBPN'
  ...
  landscape(): Observable<any> {
    const folderid='1GQoLl1UUXZOPKT8L_ykWf05nSlInyzoB'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here
  ...
  banner(): Observable<any> {
    const folderid='1v7FUHc9sFSQPMcp9SynXM_qgdPO18SSG'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here

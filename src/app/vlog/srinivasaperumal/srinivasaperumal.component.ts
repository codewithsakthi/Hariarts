...
@Component({
  selector: 'app-srinivasaperumal',
  templateUrl: './srinivasaperumal.component.html',
  styleUrls: ['./srinivasaperumal.component.css',
...
export class SrinivasaperumalComponent {
  ...
  defaultyear()
  {
    ...
    if (this.currentYear == 2022) {
      this.googleDriveService.setFolderId("1jJEAVucjRVQmspCG6ZAFZJWOJ-tWLirx");
      this.yearselcted = true;
      this.validyear=true

    }
    if (this.currentYear == 2023) {
      this.googleDriveService.setFolderId("16QFbQpNyBsYsVfgnWr263JtfjojP0Fy3");
      this.yearselcted = true;
      this.validyear=true
    ...
    if (this.currentYear == 2024) {
      this.googleDriveService.setFolderId("1W4_Tt1UVk-zXZuxvnUpOOCNlKUtKzh1D");
      this.validyear = true

    }
    if (this.currentYear == 2025) {
      this.googleDriveService.setFolderId("1tU7FQNcRb6BVtMDXxUONCbW-gWKAJL58");
      this.validyear = true

    }
    if (this.currentYear == 2026) {
      this.googleDriveService.setFolderId("1ck7YbL-RLCyMKHYC43LiMl6NROu34Hzg");
      this.validyear = true

    }
  }
  fetchBanner(): void {
    this.googleDriveService.setFolderId('17Wyqd8hsgDOw0yQz4i2IBN0j-XsEJEKu')
    this.banner().subscribe((response: any) => {
      this.data = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1vrXqNKpQlPT_2bcAugFQ8ArFZDRS9uvY')
    this.portrait().subscribe((response: any) => {
      this.Banner = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1QaBeb0pqXblSDqOWbufsqL3QlN75cBL-')
  ...
  portrait(): Observable<any> {
    const folderid='1vrXqNKpQlPT_2bcAugFQ8ArFZDRS9uvY'
  ...
  landscape(): Observable<any> {
    const folderid='1QaBeb0pqXblSDqOWbufsqL3QlN75cBL-'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here
  ...
  banner(): Observable<any> {
    const folderid='17Wyqd8hsgDOw0yQz4i2IBN0j-XsEJEKu'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here

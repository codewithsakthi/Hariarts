...
@Component({
  ...
  styleUrls: ['./krishnan.component.css',
...
export class KrishnanComponent {
 slider: KeenSliderInstance | null = null;
  ...
  defaultyear()
  {
    ...
    if (this.currentYear == 2022) {
      this.googleDriveService.setFolderId("1dnDrMK4U6yQQnXCafr9kbmE2Op8p1kIy");
      this.yearselcted = true;
      this.validyear=true

    }
    if (this.currentYear == 2023) {
      this.googleDriveService.setFolderId("1LNOP2YnOTMm9Vd_HzEOJbe-wc5OH4yit");
      this.yearselcted = true;
      this.validyear=true
    ...
    if (this.currentYear == 2024) {
      this.googleDriveService.setFolderId("1NzD6Xe9FqaSMOtPt-KYNG_uv0TXgehoi");
      this.validyear = true

    }
    if (this.currentYear == 2025) {
      this.googleDriveService.setFolderId("1T8tKJuJ7LN47yGy5zfUKaPfOkY6oRlhM");
      this.validyear = true

    }
    if (this.currentYear == 2026) {
      this.googleDriveService.setFolderId("1l_gtEOUgCdJEmAHE38QqJnSi7_O361nu");
      this.validyear = true

    }
  }
  fetchBanner(): void {
    this.googleDriveService.setFolderId('16P8Xf2ojcouRsNEKOjNaZ7UVesMlq_ho')
    this.banner().subscribe((response: any) => {
      this.data = this.sortImagesByFilename(response.files);
      console.log("hi",this.data);
    });
    this.googleDriveService.setFolderId('1skGkPkTyaVDrC0nkwfIHH6AC-IRNYXEg')
    this.portrait().subscribe((response: any) => {
      this.Banner = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1Dq7XpVovWrWAxvYPItgE59cDtOlBsDlM')
  ...
  portrait(): Observable<any> {
    const folderid='1skGkPkTyaVDrC0nkwfIHH6AC-IRNYXEg'
  ...
  landscape(): Observable<any> {
    const folderid='1Dq7XpVovWrWAxvYPItgE59cDtOlBsDlM'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here
  ...
  banner(): Observable<any> {
    const folderid='16P8Xf2ojcouRsNEKOjNaZ7UVesMlq_ho'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here

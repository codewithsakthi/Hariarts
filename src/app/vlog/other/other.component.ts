 ...
 slider: KeenSliderInstance | null = null;
  ...
  defaultyear()
  {
    ...
    if (this.currentYear == 2022) {
      this.googleDriveService.setFolderId("1QQMXjjBowndBuB_-RaTNkjg1bcYYAh6L");
      this.yearselcted = true;
      this.validyear=true

    }
    if (this.currentYear == 2023) {
      this.googleDriveService.setFolderId("10_Ow5gGHnF0IND5FFh4u-xH3_GdZ1Zhc");
      this.yearselcted = true;
      this.validyear=true
    ...
    if (this.currentYear == 2024) {
      this.googleDriveService.setFolderId("1bdJrfc5o_qh3dqNCwugPbn0sAwssLjuP");
      this.validyear = true

    }
    if (this.currentYear == 2025) {
      this.googleDriveService.setFolderId("1JH9BAYrJ_c4qanzAjSvjkem1ky1Ad62e");
      this.validyear = true

    }
    if (this.currentYear == 2026) {
      this.googleDriveService.setFolderId("1kM-0vglTK-mp3rs422Aml1A8yyJqvh59");
      this.validyear = true

    }
  }
  fetchBanner(): void {
    this.googleDriveService.setFolderId('1IIxOrmJIRwXt_HLhFhPCWgyI9WfAL_EF')
    this.banner().subscribe((response: any) => {
      this.data = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1kVotE-zi8fKU22Vzj6oXAtCalJHMbdIj')
    this.portrait().subscribe((response: any) => {
      this.Banner = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1-2SAn0mttXMgHsbe4Z_WWnaGTUP3G3BN')
  ...
  portrait(): Observable<any> {
    const folderid='1kVotE-zi8fKU22Vzj6oXAtCalJHMbdIj'
  ...
  landscape(): Observable<any> {
    const folderid='1-2SAn0mttXMgHsbe4Z_WWnaGTUP3G3BN'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here
  ...
  banner(): Observable<any> {
    const folderid='1IIxOrmJIRwXt_HLhFhPCWgyI9WfAL_EF'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here

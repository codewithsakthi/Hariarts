...
@Component({
  selector: 'app-kulasai',
  templateUrl: './kulasai.component.html',
  styleUrls: ['./kulasai.component.css',
...
export class KulasaiComponent{
  ...
  defaultyear()
  {
    ...
    if (this.currentYear == 2022) {
      this.googleDriveService.setFolderId("1Ovyoe24Gdq0Q9SHWLJPuji3IhqVvy2Xw");
      this.yearselcted = true;
      this.validyear=true

    }
    if (this.currentYear == 2023) {
      this.googleDriveService.setFolderId("1bSdVCHM3YhI42TaiffCUesxORBOptvN5");
      this.yearselcted = true;
      this.validyear=true
    ...
    if (this.currentYear == 2024) {
      this.googleDriveService.setFolderId("1fqvfS2pwJO-eVrmKovVfvnWt8i3xZ6Ox");
      this.validyear = true

    }
    if (this.currentYear == 2025) {
      this.googleDriveService.setFolderId("1FJaAN_1UOFZJ7x1AHVV3mPLjFmgA5GD3");
      this.validyear = true

    }
    if (this.currentYear == 2026) {
      this.googleDriveService.setFolderId("17Ox17dW9auM6P8z79GrHI_hFCLAZh9ps");
      this.validyear = true

    }
  }
  fetchBanner(): void {
    this.googleDriveService.setFolderId('1KaV4Vi-0WCG_QYehcLUE_-mtB4p8blsT')
    this.banner().subscribe((response: any) => {
      this.data = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1I0m5YTpBKDJT7A-ZKxOwZrD9-zXci1MC')
    this.portrait().subscribe((response: any) => {
      this.Banner = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1GTufRh1HAQiMTidgKgV7Qsl61XAoSy72')
  ...
  portrait(): Observable<any> {
    const folderid='1I0m5YTpBKDJT7A-ZKxOwZrD9-zXci1MC'
  ...
  landscape(): Observable<any> {
    const folderid='1GTufRh1HAQiMTidgKgV7Qsl61XAoSy72'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here
  ...
  banner(): Observable<any> {
    const folderid='1KaV4Vi-0WCG_QYehcLUE_-mtB4p8blsT'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here

...
@Component({
  selector: 'app-koodal',
  templateUrl: './koodal.component.html',
  styleUrls: ['./koodal.component.css',
 ...
 slider: KeenSliderInstance | null = null;
  ...
  defaultyear()
  {
    ...
    if (this.currentYear == 2022) {
      this.googleDriveService.setFolderId("1o8ohqYDNxsK-4QHMjXImmPW0S-W0CXCa");
      this.yearselcted = true;
      this.validyear=true

    }
    if (this.currentYear == 2023) {
      this.googleDriveService.setFolderId("17n6zwIwb_mW3_SHcO3G6Z46pTPlDFQck");
      this.yearselcted = true;
      this.validyear=true
    ...
    if (this.currentYear == 2024) {
      this.googleDriveService.setFolderId("15buo_Qzf9L57bgI72mM8Ocq4_HLGu5lz");
      this.validyear = true

    }
    if (this.currentYear == 2025) {
      this.googleDriveService.setFolderId("1av8aFF-8dZBIVLX0XzcIbWPXRUv8dNf1");
      this.validyear = true

    }
    if (this.currentYear == 2026) {
      this.googleDriveService.setFolderId("1HU7kw6ynPapBfGpidodkiMovuCz8gdcc");
      this.validyear = true

    }
  }
  fetchBanner(): void {
    this.googleDriveService.setFolderId('101EotR2MAvN8nJ-0xcLd75AjS65n0VYj')
    this.banner().subscribe((response: any) => {
      this.data = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1pREzuuiLbJLkI9EOm5xJcEyKvEZjH2Oe')
    this.portrait().subscribe((response: any) => {
      this.Banner = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1ZB_lHWOx7YZHxqHcH3vstbFwWz6Sr0gw')
  ...
  portrait(): Observable<any> {
    const folderid='1pREzuuiLbJLkI9EOm5xJcEyKvEZjH2Oe'
  ...
  landscape(): Observable<any> {
    const folderid='1ZB_lHWOx7YZHxqHcH3vstbFwWz6Sr0gw'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here
  ...
  banner(): Observable<any> {
    const folderid='101EotR2MAvN8nJ-0xcLd75AjS65n0VYj'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here

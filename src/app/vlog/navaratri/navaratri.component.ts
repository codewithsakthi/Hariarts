...
@Component({
  selector: 'app-navaratri',
  templateUrl: './navaratri.component.html',
  styleUrls: ['./navaratri.component.css',
...
export class NavaratriComponent {
  ...
  defaultyear()
  {
    ...
    if (this.currentYear == 2022) {
      this.googleDriveService.setFolderId("1Sn_hptZ69atxCNKoTJLvx692eLDy6ovL");
      this.yearselcted = true;
      this.validyear=true

    }
    if (this.currentYear == 2023) {
      this.googleDriveService.setFolderId("1DUwv_zCw2Vt97VYKgX2FBpRsByc_UOio");
      this.yearselcted = true;
      this.validyear=true
    ...
    if (this.currentYear == 2024) {
      this.googleDriveService.setFolderId("14haHCbyjzLZpyqhdIrvoRjoRpuk9-Ao6");
      this.validyear = true

    }
    if (this.currentYear == 2025) {
      this.googleDriveService.setFolderId("1QQLDnemQKq1cdpw36Z1zKRjndwQcofBx");
      this.validyear = true

    }
    if (this.currentYear == 2026) {
      this.googleDriveService.setFolderId("1QIgtJ5LUA2TwQ80SZeKxqNhK_4Lvj-Up");
      this.validyear = true

    }
  }
  fetchBanner(): void {
    this.googleDriveService.setFolderId('1OeC49n0dvJqpD3jLIQPrvudOq-vCxk3I')
    this.banner().subscribe((response: any) => {
      this.data = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('14aBM722mKnxmu38YJytPUKW5GPRBCK7n')
    this.portrait().subscribe((response: any) => {
      this.Banner = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1Z7x-Vi-Yf8dgjRvNZjy3B-9qBrffm6dc')
  ...
  portrait(): Observable<any> {
    const folderid='14aBM722mKnxmu38YJytPUKW5GPRBCK7n'
  ...
  landscape(): Observable<any> {
    const folderid='1Z7x-Vi-Yf8dgjRvNZjy3B-9qBrffm6dc'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here
  ...
  banner(): Observable<any> {
    const folderid='1OeC49n0dvJqpD3jLIQPrvudOq-vCxk3I'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here

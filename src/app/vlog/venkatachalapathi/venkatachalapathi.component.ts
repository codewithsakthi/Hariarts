...
@Component({
  selector: 'app-venkatachalapathi',
  templateUrl: './venkatachalapathi.component.html',
  styleUrls: ['./venkatachalapathi.component.css',
...
export class VenkatachalapathiComponent{
  ...
  defaultyear()
  {
    ...
    if (this.currentYear == 2022) {
      this.googleDriveService.setFolderId("1o2QnToGfaA6k4TcxR6kxP_pidsW-0-Sl");
      this.yearselcted = true;
      this.validyear=true

    }
    if (this.currentYear == 2023) {
      this.googleDriveService.setFolderId("1IzSzV2omm2R80C-HkBueRgynL3e5h-1t");
      this.yearselcted = true;
      this.validyear=true
    ...
    if (this.currentYear == 2024) {
      this.googleDriveService.setFolderId("1pt36UczwPawpbd4TckRQg827_emCBs6C");
      this.validyear = true

    }
    if (this.currentYear == 2025) {
      this.googleDriveService.setFolderId("1I16mmKEk6PQFAx5PJ7n-P8QCgAyQ04ej");
      this.validyear = true

    }
    if (this.currentYear == 2026) {
      this.googleDriveService.setFolderId("1Lm7CFcYppCdhnhTs5i_uhvsIiQh9G8ZX");
      this.validyear = true

    }
  }
  fetchBanner(): void {
    this.googleDriveService.setFolderId('1BDTwcKA3Rwpia3BTBWdIxKstWczMztSK')
    this.banner().subscribe((response: any) => {
      this.data = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('12GlUK_W3VbQypeXZ1jU9h7jy3qcOOOw5')
    this.portrait().subscribe((response: any) => {
      this.Banner = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('15nYK9rIjEUcfnwBIKsa07rGXrK4JCPs7')
  ...
  portrait(): Observable<any> {
    const folderid='12GlUK_W3VbQypeXZ1jU9h7jy3qcOOOw5'
  ...
  landscape(): Observable<any> {
    const folderid='15nYK9rIjEUcfnwBIKsa07rGXrK4JCPs7'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here
  ...
  banner(): Observable<any> {
    const folderid='1BDTwcKA3Rwpia3BTBWdIxKstWczMztSK'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here

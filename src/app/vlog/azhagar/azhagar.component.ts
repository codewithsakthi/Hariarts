...
@Component({
  selector: 'app-azhagar',
  templateUrl: './azhagar.component.html',
  styleUrls: ['./azhagar.component.css',
 ...
 slider: KeenSliderInstance | null = null;
  ...
  defaultyear()
  {
    ...
    if (this.currentYear == 2022) {
      this.googleDriveService.setFolderId("11wlB6SOnRC4Okn6DUep6Q6r7db3HHAIZ");
      this.yearselcted = true;
      this.validyear=true

    }
    if (this.currentYear == 2023) {
      this.googleDriveService.setFolderId("1Is6D70rLeyBWA74E-983EJwIOY4_hTzX");
      this.yearselcted = true;
      this.validyear=true
    ...
    if (this.currentYear == 2024) {
      this.googleDriveService.setFolderId("1njtBzSOF2pWuI-wMsyyT17K_ur-dGXh9");
      this.validyear = true

    }
    if (this.currentYear == 2025) {
      this.googleDriveService.setFolderId("1aHKo4E8ESdk6IxWKxtbSjIKkAhkMKjBx");
      this.validyear = true

    }
    if (this.currentYear == 2026) {
      this.googleDriveService.setFolderId("1YZk9lpfikyguGwShBbNmAuizPxE2wDea");
      this.validyear = true

    }
  }
  fetchBanner(): void {
    this.googleDriveService.setFolderId('1QhHMeSNM0JnrGajGMU2RYhMjyvOpz4bV')
    this.banner().subscribe((response: any) => {
      this.data = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1zo3W-O-Qi8belo5qwagyx2vS9UzyvCp1')
    this.portrait().subscribe((response: any) => {
      this.Banner = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1xhN9s3updQiuLqoYyHmV5M_2gPseGISS')
  ...
  portrait(): Observable<any> {
    const folderid='1zo3W-O-Qi8belo5qwagyx2vS9UzyvCp1'
  ...
  landscape(): Observable<any> {
    const folderid='1xhN9s3updQiuLqoYyHmV5M_2gPseGISS'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here
  ...
  banner(): Observable<any> {
    const folderid='1QhHMeSNM0JnrGajGMU2RYhMjyvOpz4bV'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here

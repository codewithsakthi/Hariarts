...
@Component({
  selector: 'app-kamakshi',
  templateUrl: './kamakshi.component.html',
  styleUrls: ['./kamakshi.component.css',
...
export class KamakshiComponent{
 slider: KeenSliderInstance | null = null;
  ...
  defaultyear()
  {
    ...
    if (this.currentYear == 2022) {
      this.googleDriveService.setFolderId("10hUaiw1RHcdFV2zQQEfff_uImWa9rCA-");
      this.yearselcted = true;
      this.validyear=true

    }
    if (this.currentYear == 2023) {
      this.googleDriveService.setFolderId("1ZezvRQkOg13KHLcWlUGs4LfZr0wrxbZT");
      this.yearselcted = true;
      this.validyear=true
    ...
    if (this.currentYear == 2024) {
      this.googleDriveService.setFolderId("1-_kkNPPjixA8LIAlGgbu6Y7tePrBKZCy");
      this.validyear = true

    }
    if (this.currentYear == 2025) {
      this.googleDriveService.setFolderId("14Q77hLDGo3M3Ahd_DkaRFDMOuqdH7gxC");
      this.validyear = true

    }
    if (this.currentYear == 2026) {
      this.googleDriveService.setFolderId("1IKXdMg4BEddgMfVv8D7FXO94YOXzb5XO");
      this.validyear = true

    }
  }
  fetchBanner(): void {
    this.googleDriveService.setFolderId('15CqfXuNKznt-IgW7-KA7axkCtnNwmfaR')
    this.banner().subscribe((response: any) => {
      this.data = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1M7sU0VylBx4uQiHXDE0q60j7FDdsg1op')
    this.portrait().subscribe((response: any) => {
      this.Banner = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1ZPD34Q1_v_QJ1VLPHIjyebWH7MTtT9a6')
  ...
  portrait(): Observable<any> {
    const folderid='1M7sU0VylBx4uQiHXDE0q60j7FDdsg1op'
  ...
  landscape(): Observable<any> {
    const folderid='1ZPD34Q1_v_QJ1VLPHIjyebWH7MTtT9a6'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here
  ...
  banner(): Observable<any> {
    const folderid='15CqfXuNKznt-IgW7-KA7axkCtnNwmfaR'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here

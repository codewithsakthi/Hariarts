 ...
 slider: KeenSliderInstance | null = null;
  ...
  defaultyear()
  {
    ...
    if (this.currentYear == 2022) {
      this.googleDriveService.setFolderId("1t7gp_WHBKVlH0tnK1tdwQUUzFZWhNTMq");
      this.yearselcted = true;
      this.validyear=true

    }
    if (this.currentYear == 2023) {
      this.googleDriveService.setFolderId("1T9D3Tfyrei-sCUUsfUT06fqzDSHKjsdg");
      this.yearselcted = true;
      this.validyear=true
    ...
    if (this.currentYear == 2024) {
      this.googleDriveService.setFolderId("1py0on-DKrspMV6GY73mCxj9Z1emqpVnL");
      this.validyear = true

    }
    if (this.currentYear == 2025) {
      this.googleDriveService.setFolderId("1tyZtlRNtUfMocpB_0pB_k6_pGuVDFnWE");
      this.validyear = true

    }
    if (this.currentYear == 2026) {
      this.googleDriveService.setFolderId("172cfDgqQeqV_sgeVwcQgydvnbEPF_m8B");
      this.validyear = true

    }
  }
  fetchBanner(): void {
    this.googleDriveService.setFolderId('1wBVD36AXabcZ6SUHmQSVQMpaaBJ6j54m')
    this.banner().subscribe((response: any) => {
      this.data = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1uGZPQ3vQd-SJlY8H7pqM5JWRbHVEZZAM')
    this.portrait().subscribe((response: any) => {
      this.Banner = response.files;
      console.log(this.data);
    });
    this.googleDriveService.setFolderId('1zJQZdKDSJ_HddMpV6jG1N7ehhtSl7hTn')
  ...
  portrait(): Observable<any> {
    const folderid='1uGZPQ3vQd-SJlY8H7pqM5JWRbHVEZZAM'
  ...
  landscape(): Observable<any> {
    const folderid='1zJQZdKDSJ_HddMpV6jG1N7ehhtSl7hTn'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here
  ...
  banner(): Observable<any> {
    const folderid='1wBVD36AXabcZ6SUHmQSVQMpaaBJ6j54m'
    const apiUrl = 'https://www.googleapis.com/drive/v3/files';
    const params = {
      q: `'${folderid}' in parents and mimeType!='application/vnd.google-apps.folder'`, // Filters out folders
      key: this.googleDriveService.API_KEY, // Include your API key here
